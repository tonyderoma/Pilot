...
public abstract class DaoHelperBaseResult2 extends PilotSupport implements Serializable {
	...
	private void logNoQuery() {
		log("Occorre definire una query di select da eseguire. La query attuale è vuota, non esiste alcuna key ", getQueryName(), " nel file ", QUERY_FILE);
	}
	...
	public <K> PList<K> selectPaginatedBean(Class<K> c, Integer numeroPagina, Integer quantiPerPagina, Object... params) throws Exception {
		...
		String error = null;
		if (isPagesPresent()) {
			recordTotali = selectOneDirectQuery(formaStringaCount(getQuery()), BigDecimal.class, params);
		...
		try {
			setQuery(addPaginationClause(getQuery(), new Pagination(numeroPagina, quantiPerPagina)));
			stmt = getConnection().prepareStatement(removeNull(params));
			...
			impostaParametriInTestoQuery(params);
			if (notNull(error))
				logQuery(getQuery(), start, end, elenco.size(), error);
			else
				logQuery(getQuery(), start, end, elenco.size());
			closeAlls(stmt, rs);
		}
		return elenco;
	}
	...
	public <K> PList<K> selectPaginatedBeanDTO(Class<K> c, Integer numeroPagina, Integer quantiPerPagina, Object o) throws Exception {
		...
		String error = null;
		if (isPagesPresent()) {
			recordTotali = selectOneDirectQueryDTO(formaStringaCount(getQuery()), o);
			Long resto = recordTotali.longValue() % quantiPerPagina;
			Long quantePagine = dividi(recordTotali, getBigDecimal(quantiPerPagina)).longValue();
			if (resto > 0)
				quantePagine++;
		...
		try {
			setQuery(addPaginationClause(getQuery(), new Pagination(numeroPagina, quantiPerPagina)));
			params = toArray(extractParams(o), Object.class);
			stmt = getConnection().prepareStatement(removeNull(params));
			bindParams(stmt, params);
			stmt.setQueryTimeout(getQueryTimeout());
			start = now();
				...
				if (is(method.getName(), str(SET, col))) {
					...
					for (int i = 1; i <= rsmd.getColumnCount(); i++) {
						if (tutte(is(rsmd.getColumnName(i), col))) {
							if (tutte(is(rsmd.getColumnTypeName(i), NUMBER, INTEGER, LONG), zero(rsmd.getScale(i)))) {
								try {
	...
	private <K> K selectOneDirectQuery(String sql, Object... params) throws Exception {
		...
		try {
			...
			impostaParametriInTestoQuery(params);
			if (notNull(error))
				logQuery(getQuery(), start, end, 0, error);
			else
				logQuery(getQuery(), start, end, 1);
			closeAlls(stmt, rs);
	 ...
	 * definiti attraverso la dichiarazione di variabili istanza annotate con @OutParam
	 * dichiarate nello stesso ordine di definizione degli out parameters della
	 * stored procedure. Tali variabili istanza al termine dell'invocazione
	 * conterranno i valori restituiti dalla SP e i cui tipi Java sono dello
	 * stesso tipo che prevede la SP di restituire in uscita come out parameter.
	 * Esempio di definizione della invocazione di SP con parametri di IN e OUT
	 * in cui il secondo ? è un out parameter: feriale={call
	 * DBK_ENEK_UTILITY.DBP_ENEK_GetFeriale( ? ,? )}. Se il tipo Out Parameter è
	 * un Date, occorre definire nella classe una variabile del tipo:
	 * 
	 ...
	 * stesso tipo che prevede la SP di restituire in uscita come out parameter.
	 * Esempio di definizione della invocazione di SP con parametri di IN e OUT:
	 * feriale={call DBK_ENEK_UTILITY.DBP_ENEK_GetFeriale( ? )}. Se il tipo Out
	 * Parameter è un Date, occorre definire nella classe una variabile del
	 * tipo:
	 * 
	 * OutParam private Date data;
	 * 
	...
	public Long selectCount(Object... params) throws Exception {
		...
		try {
			...
			impostaParametriInTestoQuery(params);
			if (notNull(error))
				logQuery(getQuery(), start, end, quanti.intValue(), error);
			else
				logQuery(getQuery(), start, end, quanti.intValue());
			closeAlls(stmt, rs);
		}
		return quanti;
	}
	...
	private <K> PList<K> _selectNoBean(Class<K> c, Object... params) throws Exception {
		...
		try {
			...
			impostaParametriInTestoQuery(params);
			if (notNull(error))
				logQuery(getQuery(), start, end, item.size(), error);
			else
				logQuery(getQuery(), start, end, item.size());
			closeAlls(stmt, rs);
		}
		return item;
	}
	...
	private PList<Object> extractParams(Object o) throws Exception {
		Field[] attributi = o.getClass().getDeclaredFields();
		PList<Object> params = pl();
		for (Field att : attributi) {
			params.add(get(o, att.getName()));
		}
		return params;
	}
	...
	private String buildSqlDTO(String sql, Object o) throws Exception {
		...
		for (Map.Entry<String, Object> entry : mappa.entrySet()) {
			...
			if (notNull(val)) {
				...
				// valore del parametro è NULL (vedi query per filtri di
	...
	private void bindParams(PreparedStatement ps, Object... params) throws SQLException {
		...
		for (Object par : params) {
			if (Null(par))
				continue;
			i++;
			if (par instanceof List) {
				par = toArray((List) par, ((List) par).get(0).getClass());
			}
			if (par instanceof String[]) {
				ps.setArray(i, getConnection().createArrayOf("VARCHAR", (String[]) par));
				continue;
			}

			if (par instanceof Integer[]) {
				ps.setArray(i, getConnection().createArrayOf("INTEGER", (Integer[]) par));
				continue;
			}

			if (par instanceof Double[]) {
				ps.setArray(i, getConnection().createArrayOf("DOUBLE", (Double[]) par));
				continue;
			}

			if (par instanceof Date[]) {
				ps.setArray(i, getConnection().createArrayOf("DATE", (Date[]) par));
				continue;
			}

			if (par instanceof String) {
				ps.setString(i, (String) par);
				continue;
			}
			if (par instanceof Integer) {
				ps.setInt(i, (Integer) par);
				continue;
			}
			if (par instanceof Long) {
				ps.setLong(i, (Long) par);
				continue;
			}
			if (par instanceof Double) {
				ps.setDouble(i, (Double) par);
				continue;
			}
			if (par instanceof Float) {
				ps.setFloat(i, (Float) par);
				continue;
			}
			if (par instanceof BigDecimal) {
				ps.setBigDecimal(i, (BigDecimal) par);
				continue;
			}
			if (par instanceof Date) {
				ps.setDate(i, getSQLDate((Date) par));
				continue;
			}
			if (par instanceof Timestamp) {
				ps.setTimestamp(i, (Timestamp) par);
				continue;
			}
	...
	private PList<Integer> getPosizioniNull(Object... params) {
		PList<Integer> pos = pl();
		int i = 0;
	...
	private String removeNullDTO(Object... params) throws Exception {
		String query = despaceString(getQuery()).toUpperCase();

		String[] elems = query.split("\\?");
		if (params.length < elems.length - 1)
			throw new Exception(strSpaced("Passati", params.length, "parametri mentre ne sono previsti", elems.length - 1,
					". I parametri eccessivi vengono automaticamente esclusi ma occorrono almeno", elems.length - 1, " parametri secondo l'ordine di dichiarazione nel dto"));
		List<Integer> pos = getPosizioniNull(Arrays.copyOfRange(params, 0, elems.length - 1));
		log(pos);
		if (Null(pos)) {
			log(query);
			return query;
		...
		PList<Integer> indiciTondeDaRimuovere = pl();
		for (Integer i : safe(pos)) {
			elems[i] = elems[i].trim();

			int and = elems[i].lastIndexOf("AND ");
			int or = elems[i].lastIndexOf("OR ");
			int where = elems[i].lastIndexOf(WHERE);
			if (where > -1) {
				where = where + WHERE.length();
			}
			if (isLike(elems[i], "IN (")) {
				indiciTondeDaRimuovere.add(i + 1);// dalla stringa successiva
			}
			elems[i] = elems[i].substring(0, max(and, or, where));
		...
		for (int i = 0; i < elems.length; i++) {
			if (Null(elems[i]))
				continue;
			if (is(indiciTondeDaRimuovere, i)) {
				elems[i] = elems[i].trim();
				elems[i] = elems[i].substring(1, elems[i].length());
			}
			if (i == elems.length - 1) {
				nuova = str(nuova, elems[i]);
			} else if (endsWith(elems[i], WHERE)) {
				nuova = str(nuova, elems[i]);
			} else {
				if (isNot(pos, i)) {
					nuova = str(nuova, elems[i], " ? ");
		...
		nuova = despaceString(nuova);
		nuova = replace(nuova, "WHERE OR", WHERE);
		nuova = replace(nuova, "WHERE AND", WHERE);
		if (endsWith(nuova, WHERE)) {
			nuova = replace(nuova, WHERE, "");
		}
		return nuova;

	}

	private PList<Integer> getPosizioniBetween(String query) throws Exception {
		PList<Integer> pos = pl();
		query = replace(query.toUpperCase(), "BETWEEN ? AND ?", "çç§");
		String[] elems = query.split("çç§");
		int conta = -1;
		for (String pezzo : arrayToList(elems).dropLast()) {
			conta = conta + countChar(pezzo, '?');
			pos.add(conta + 1);
			pos.add(conta + 2);
			conta = conta + 2;
		...
		if (pos.size() % 2 != 0)
			throw new Exception("Parametri delle clausole BETWEEN ERRATI!");
		log(pos);
		return pos;
	}

	private String removeNull(Object... params) throws Exception {
		String query = despaceString(getQuery()).toUpperCase();
		int quanti = countChar(query, '?');
		if (params.length != quanti)
			throw new Exception(strSpaced("Passati", params.length, "parametri mentre ne sono previsti", quanti));
		boolean fineConPar = endsWith(query.trim(), "?");
		PList<Integer> pos = getPosizioniNull(params);
		gestisciNullitaBetween(query, params);
		pos = pl(p.sottraiList(pos, getPosizioniBetween(query)));
		if (fineConPar) {// solo se finisco con un parametro controllo che non
			// sia nullo
			fineConPar = isNot(pos, params.length - 1);// se la lista delle
			// posizioni dei
			// parametri nulli
			// contiene l'ultimo
			// elemento, significa
			// che sto annullando
			// l'ultimo parametro e
			// quindi fine con par è
			// false
		}
		if (Null(pos)) {
			return query;
		...
		for (Integer i : safe(pos)) {
			...
			log("..............", elems[i]);
			elems[i] = elems[i].substring(0, limiteInferiore(max(and, or, where), 0));
		}
	...
	private void gestisciNullitaBetween(String query, Object[] params) throws Exception {
		int i = 0;
		for (Integer pos : safe(getPosizioniBetween(query))) {
			i++;
			if (Null(params[pos])) {
				params[pos] = (i % 2 == 0) ? getInfiniteDate() : getInfiniteStartDate();
			}
		}

	}

	private void impostaParametriInTestoQuery(Object... params) {
		if (null != params) {
			String q = getQuery();
			for (int k = params.length - 1; k >= 0; k--) {
				if (notNull(params[k])) {
					q = replaceLast(q, "?", str(params[k]));
				}
			}
			setQuery(q);
		}
	...
	private <K> K buildBean(Class<K> c, ResultSet rs, PList<String> alias, Method[] methods) throws InstantiationException, IllegalAccessException, ClassNotFoundException, InvocationTargetException,
			SQLException {
		K item = (K) Class.forName(c.getName()).newInstance();
		ResultSetMetaData rsmd = rs.getMetaData();
		for (String col : safe(alias)) {
	...
	public boolean isPossibleSqlInjection(String input) {
