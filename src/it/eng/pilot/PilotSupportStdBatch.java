...
import it.inpdap.common.batch.StdBatchJ2SE;
...
public class PilotSupportStdBatch extends StdBatchJ2SE {
	...
	public PilotSupportStdBatch() {
		aggiungiTimeCount();
	}

	public PilotSupportStdBatch(Logger log) {
		setLog(log);
	}
	...
	/**
	 * Ritorna l'istanza dell'EJBObject per eseguire una chiamata remota EJB
	 * 
	 * @param <K>
	 * @param <T>
	 * @param aliasContext
	 * @param aliasJndiName
	 * @param c
	 *            classe della HomeInterface
	 * @return T
	 */
	protected <K extends EJBHome, T extends EJBObject> T ejb(String aliasContext, String aliasJndiName, Class<K> c) {
		String name = c.getName();
		if (Null(p.ejbMap.get(name))) {
			try {
				K home = (K) (c.cast(callRemoteHome(aliasContext, aliasJndiName, c)));
				p.ejbMap.put(name, (T) home.getClass().getDeclaredMethod("create").invoke(home));
			} catch (Exception e) {
				logError("Impossibile creare", name, e);
			}
		}
		return (T) p.ejbMap.get(name);
	}

	/**
	 * Ritorna la connessione al db tramite l'alias che corrisponde a quello nel
	 * file di properties utenze.props
	 * 
	 * @param alias
	 * @return Connection
	 * @throws Exception
	 */
	protected Connection getConn(String alias) throws Exception {
